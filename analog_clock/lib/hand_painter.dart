...
class HandPainter extends TimedCustomPainter {
  ...
  static Duration _fractionOf(Duration d) =>
      Duration(milliseconds: (d.inMilliseconds / 10).floor());

  HandPainter(this.handFunction, this.time, this.duration, this.part)
      : super(_fractionOf(duration));
  ...
  void custom(Canvas canvas, Size size) {
    ...
    if (part == HandPart.hand) {
      _paintHand(canvas, center, inside, outside);
    }
    if (part == HandPart.text) {
      _paintText(canvas, size, inside);
    }
    if (part == HandPart.tail) {
      _paintTail(canvas, size, outside, length, false);
    }
    if (part == HandPart.windyTail) {
      _paintTail(canvas, size, outside, length, true);
    }
  }
  ...
  void _paintHand(Canvas canvas, Offset center, Offset inside, Offset outside) {
    ...
    canvas.drawLine(Offset(center.dx + 1, center.dy - delta), inside, black);
    canvas.drawLine(
        Offset(center.dx - delta - 3, center.dy + delta), outside, white);
    canvas.drawLine(
        Offset(center.dx + delta + 3, center.dy - delta), outside, white);
  }

  void _paintTail(
      Canvas canvas, Size size, Offset position, double length, bool windy) {
    double alpha = 180;
    DateTime t = time;
